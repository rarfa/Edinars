...
if($subscription_id=="") {
    $array_reponse['errors']['get_subscription_code'] = "Veuillez entrer le produit ID";
    $array_reponse['success'] = "no";
}elseif($subscription[0]['id']=="") {
    $array_reponse['errors']['get_subscription_code'] = "Produit n'existe pas!";
    $array_reponse['success'] = "no";
}
...
if($array_reponse['success']=="yes") {
    ...
    $type=select_type($subscription_id);
    $tname=$data['PaymentType'][$type];

    $button=select_button($subscription_id);

    $strToCrypt = "action={$tname}|" ;
    $strToCrypt .= "identifiant=".get_member_username($user_id)."|" ;
    $strToCrypt .= "produit=".$subscription_id ;
