(function ($) {
    ...
    elFinder = function (el, o) {
        ...
        this.tmb = function () {
            this.ajax(
                {cmd : 'tmb', current : self.cwd.hash}, function (data) {
        ...
        this.reload = function (data) {
            ...
            if (this.options.autoReload>0) {
                ...
                this.iID = setInterval(
                    function () {
                        !self.locked && self.ui.exec('reload'); }, this.options.autoReload*60000
                );
            }
        }
        ...
        this.drop = function (e, ui, target) {
            ...
            var ids = [];
            ui.helper.find('div:not(.noaccess):has(>label):not(:has(em[class="readonly"],em[class=""]))').each(
                function () {
            ...
            if (ids.length) {
                ...
                if (self.buffer.files) {
                    ...
                    setTimeout(
                        function () {
                            self.ui.exec('paste'); self.buffer = {}}, 300
                    );
                }
            } else {
        ...
        this.updateSelect = function () {
            self.selected = [];
            $('.ui-selected', self.view.cwd).each(
                function () {
    ...
    elFinder.prototype.view = function (fm, el) {
        ...
        this.err = $('<p class="el-finder-err"><strong/></p>').click(
            function () {
                $(this).hide(); }
        );
...
(function ($) {
    ...
    elFinder.prototype.ui.prototype.commands = {
        ...
        back : function (fm) {
            ...
            this.exec = function () {
                if (this.fm.history.length) {
                    this.fm.ajax(
                        { cmd : 'open', target : this.fm.history.pop()    }, function (data) {
                            self.fm.reload(data);
                        }
                    );
                }
        ...
        reload : function (fm) {
            ...
            this.exec = function () {
                this.fm.ajax(
                    { cmd : 'open', target : this.fm.cwd.hash, tree : true }, function (data) {
                        self.fm.reload(data);
                    }
                );
            }
           ...
           *
            ...
            this.exec = function (dir) {
                ...
                function openDir(id)
                {
                    ...
                    self.fm.ajax(
                        { cmd : 'open', target : id }, function (data) {
                            self.fm.reload(data);
                        }
                    );
                }
                    ...
                    $('<div />').append(tb+'</table>').dialog(
                        {
                            ...
                            close       : function () {
                                if (--cnt <= 0) { self.fm.lockShortcuts(); } $(this).dialog('destroy'); },
                            buttons     : { Ok : function () {
                                $(this).dialog('close'); }}
                        }
                    );
            ...
            this.exec = function () {
                ...
                function rename()
                {
                    ...
                    if (!self.fm.locked) {
                        ...
                        self.fm.ajax(
                            {cmd : 'rename', current : self.fm.cwd.hash, target : f.hash, name : name}, function (data) {
                                if (data.error) {
                                    restore();
                                } else {
                    ...
                    .dialog(
                        {
                            ...
                            buttons     : {
                                ...
                                Ok     : function () { 
                                    ...
                                    self.fm.ajax(
                                        o, function (data) {
                                            data.tree && self.fm.reload(data); }, {force : true}
                                    );
                                }
                            }
                        }
            ...
            this.exec = function () {
                ...
                function mkdir()
                {
                    if (!self.fm.locked) {
                        ...
                        self.fm.ajax(
                            {cmd : 'mkdir', current : self.fm.cwd.hash, name : name}, function (data) {
                                if (data.error) {
                                    el.addClass('ui-selected');
                                    return input.select().focus();
                                }
            ...
            this.exec = function () {
                ...
                function mkfile()
                {
                    if (!self.fm.locked) {
                        ...
                        self.fm.ajax(
                            {cmd : 'mkfile', current : self.fm.cwd.hash, name : name}, function (data) {
                                if (data.error) {
                                    el.addClass('ui-selected');
                                    return input.select().focus();
                                }
                    ...
                    function result()
                    {
                        try {
                            ...
                            if (self.fm.jquery>=141) {
                                data = $.parseJSON(html);
                            } else if (/^[\],:{}\s]*$/.test(html.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@")           .replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, "]")           .replace(/(?:^|:|,)(?:\s*\[)+/g, "")
                            ) { ) {
                                   /* get from jQuery 1.4 */
                ...
                this.fm.ajax(
                    {
                        ...
                        self.fm.lockShortcuts(true);
                        var ta= $('<textarea/>').val(data.content||'').keydown(
                            function (e) {
                                e.stopPropagation(); }
                        );
                        $('<div/>').append(ta)
...
(function ($) {
    elFinder.prototype.quickLook = function (fm, el) {
        ...
        .append(
            $('<div class="el-finder-ql-drag-handle"/>').append(
                $('<span class="ui-icon ui-icon-circle-close"/>').click(
                    function () {
                        self.hide(); }
                )
            ).append(this.title)
        ...
        image : new function () {
            ...
            this.show = function (ql, f) {
                ...
                if (ql.mimes[f.mime] && f.hash == ql._hash) {
                    $('<img/>').hide().appendTo(ql.media.show()).attr('src', f.url+($.browser.msie || $.browser.opera ? '?'+Math.random() : '')).load(
                        function () {
                             t = $(this).unbind('load');
                            if (f.hash == ql._hash) { 
                                ql.win.is(':animated') ? setTimeout(
                                    function () {
                                        preview(t); }, 330
                                ) : preview(t);
                            } 
...
(function ($) {
    elFinder.prototype.eventsManager = function (fm, el) {
        ...
        this.init = function () {
            ...
            .bind(
                window.opera?'click':'contextmenu', function (e) {
                ...
                this.fm.view.plc.droppable(
                    {
                        ...
                        drop      : function (e, ui) {
                            ...
                            ui.helper.children('.directory:not(.noaccess,.dropbox)').each(
                                function () {
            ...
            $(document).bind(
                $.browser.mozilla || $.browser.opera ? 'keypress' : 'keydown', function (e) {
            ...
            $(document).bind(
                $.browser.opera ? 'keypress' : 'keydown', function (e) {
                    if (self.lock) {
                        return;
                    }
                    switch(e.keyCode) {
            ...
            $('a:not(.noaccess,.readonly)', this.tree).droppable(
                {
                    ...
                    out    : function () {
                        $(this).removeClass('el-finder-droppable'); },
                    drop   : function (e, ui) {
                        $(this).removeClass('el-finder-droppable'); self.fm.drop(e, ui, $(this).attr('key')); }
                }
            );
            ...
            .draggable(
                {
                    ...
                    helper     : function () {
                        ...
                        self.cwd.find('.ui-selected').each(
                            function (i) {
