d3 = function () {
    ...
    d3.behavior.drag = function () {
        ...
        function mousedown()
        {
            ...
            function point()
            {
                ...
                return touchId != null ? d3.touches(p).filter(
                    function (p) {
    ...
    d3_selection_onFilters.forEach(
        function (k) {
    ...
    d3.xhr = function (url, mimeType, callback) {
        ...
        xhr.send = function (method, data, callback) {
            ...
            if (callback != null) { xhr.on("error", callback).on(
    ...
    function d3_dsv(delimiter, mimeType)
    {
        ...
        dsv.format = function (rows) {
            ...
            return [ fields.map(formatValue).join(delimiter) ].concat(
                rows.map(
                    function (row) {
                            return fields.map(
                                function (field) {
    ...
    {
        return d3_geo_projectionMutator(
            function () {
                return project;
            }
        )();
    ...
    d3.geo.graticule = function () {
        ...
        function lines()
        {
            return d3.range(Math.ceil(X0 / DX) * DX, X1, DX).map(X).concat(d3.range(Math.ceil(Y0 / DY) * DY, Y1, DY).map(Y)).concat(
                d3.range(Math.ceil(x0 / dx) * dx, x1, dx).filter(
                    function (x) {
                        return Math.abs(x % DX) > ε;
                    }
                ).map(x)
            ).concat(
                d3.range(Math.ceil(y0 / dy) * dy, y1, dy).filter(
                    function (y) {
    ...
    d3_svg_lineInterpolators.forEach(
        function (key, value) {
            value.key = key;
    ...
    {
        return points.length < 3 ? d3_svg_lineLinear(points) : points[0] + d3_svg_lineHermite(
    ...
    d3.geom.voronoi = function (points) {
        ...
        function voronoi(data)
        {
            ...
            polygons = polygons.map(
                function (polygon, i) {
                    var cx = points[i][0], cy = points[i][1], angle = polygon.map(
                        function (v) {
                            return Math.atan2(v[0] - cx, v[1] - cy);
                        }
                    ), order = d3.range(polygon.length).sort(
                        function (a, b) {
        ...
        voronoi.triangles = function (data) {
            ...
            return d3.geom.delaunay(points).map(
                function (triangle) {
                    return triangle.map(
                        function (point) {
                            return point.data;
    ...
    d3.layout.pie = function () {
        ...
        function pie(data)
        {
            ...
            if (sort != null) { index.sort(
                sort === d3_layout_pieSortByValue ? function (i, j) {
    ...
    var d3_layout_stackOrders = d3.map(
        {
            "inside-out": function (data) {
                var n = data.length, i, j, max = data.map(d3_layout_stackMaxIndex), sums = data.map(d3_layout_stackReduceSum), index = d3.range(n).sort(
                    function (a, b) {
                        return max[a] - max[b];
                    }
    ...
    d3.layout.tree = function () {
        ...
        function tree(d, i)
        {
            ...
            d3_layout_treeVisitAfter(
                root, function (node, previousSibling) {
                    node._tree = {
                        ancestor: node,
                        prelim: 0,
                        mod: 0,
    ...
    function d3_scale_linearTickFormat(domain, m, format)
    {
        ...
        return d3.format(
            format ? format.replace(
                d3_format_re, function (a, b, c, d, e, f, g, h, i, j) {
    ...
    function d3_scale_ordinal(domain, ranger)
    {
        ...
        {
            return d3.range(domain.length).map(
                function (i) {
                    return start + step * i;
                }
            );
    ...
    function d3_transition_tween(groups, name, value, tween)
    {
        var id = groups.id;
        return d3_selection_each(
            groups, typeof value === "function" ? function (node, i, j) {
    ...
    d3_transitionPrototype.attr = function (nameNS, value) {
        ...
        return d3_transition_tween(
            this, "attr." + nameNS, value, function (b) {
                function attrString()
                {
    ...
    d3_transitionPrototype.style = function (name, value, priority) {
        ...
        return d3_transition_tween(
            this, "style." + name, value, function (b) {
    ...
    d3_transitionPrototype.delay = function (value) {
        var id = this.id;
        return d3_selection_each(
            this, typeof value === "function" ? function (node, i, j) {
    ...
    var d3_time_scaleUTCMethods = d3_time_scaleLocalMethods.map(
        function (m) {
            return [ m[0].utc, m[1] ];
        }
    );
