    ...
    <div class="admin_box_header">
    	<div class="box_header_left">
    		<div class="header_tabs">
				...
				<? } else { ?>
					<a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$post['StartPage']?>" class="no_submenu"><span class="tab">LES TRANSACTIONS</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
				<?}?>
				   ...
				   <?}else{?>
					    <a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$post['type']?>/0" class="no_submenu"><span class="tab">EN ATTENTE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
					<?}?> 
					<?if($post['status']==1){?>
						<a href="#tab01" class="default_tab current">
						<span class="tab">
					...
					<?}else{?>
					    <a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$post['type']?>/1" class="no_submenu"><span class="tab">TERMINE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
				    <?}?> 
					<?if($post['status']==2){?>
						<a href="#tab01" class="default_tab current">
						<span class="tab">
					...
					<?}else{?>
					    <a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$post['type']?>/2" class="no_submenu"><span class="tab">ANNULER</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
					<?}?> 
					<?if($post['status']==3){?>
						<a href="#tab01" class="default_tab current">
						<span class="tab">
					...
					<?}else{?>
						<a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$post['type']?>/3" class="no_submenu"><span class="tab">REMBOURSE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
					<?}?>
									...
									<tr>
										...
										<?if($value['canview']){?>
											<a href="<?=$data['Members']?>/mon-historique-Edinars-transaction.html/<?=$value['id']?>/details">D&Egrave;TAIL</a><?}?>
												<?if($value['canrefund']){?>
													<br><a href="<?=$data['Members']?>/mon-historique-Edinars-transaction.html/<?=$value['id']?>/refund" onclick="return cfmform()">REMBOURSE</a>
												<?}?>
										</td>
									</tr>
						     <?php 
					...
					<div class="page_area">
						<?$count=count($data['Pages']);
							 for($i=0; $i<$count; $i++){?>
								...
								<?}else{?>
									<a href="<?=$data['Members']?>/mon-historique-Edinars.html/<?=$data['Pages'][$i]?>/<?=$post['type']?>/<?=$post['status']?>/transaction" title="Page <?=$i+1?>"><?=$i+1?></a>
								<?}?>
							<?}?>
					</div>			
				<?}?>
									...
									<tr>	
										...
										<td>	
											<?if($post['Transaction']['userid']>0){?>
												<a href="userinfo.html?id=<?=$post['Transaction']['userid']?><?if(isset($post['StartPage'])){?>&page=<?=$post['StartPage']?><?}?>&action=view"><?=$post['Transaction']['username']?></a>
											<?}else{?>
