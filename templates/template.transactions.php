    ...
    <div class="admin_box_header">
        <div class="box_header_left">
            <div class="header_tabs">
                ...
                <? } else { ?>
                    <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $post['StartPage']?>" class="no_submenu"><span class="tab">LES TRANSACTIONS</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
                <?}?>
                   ...
                   <?}else{?>
                        <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $post['type']?>/0" class="no_submenu"><span class="tab">EN ATTENTE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
                    <?}?> 
                    <?if($post['status']==1){?>
                        <a href="#tab01" class="default_tab current">
                        <span class="tab">
                    ...
                    <?}else{?>
                        <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $post['type']?>/1" class="no_submenu"><span class="tab">TERMINE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
                    <?}?> 
                    <?if($post['status']==2){?>
                        <a href="#tab01" class="default_tab current">
                        <span class="tab">
                    ...
                    <?}else{?>
                        <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $post['type']?>/2" class="no_submenu"><span class="tab">ANNULER</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
                    <?}?> 
                    <?if($post['status']==3){?>
                        <a href="#tab01" class="default_tab current">
                        <span class="tab">
                    ...
                    <?}else{?>
                        <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $post['type']?>/3" class="no_submenu"><span class="tab">REMBOURSE</span><span class="tab_right"></span></a> <!-- add class="no_submenu" while a tab with no content -->
                    <?}?>
                        ...
                        <tbody>
                                <?php if($post['Transactions']) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?if($value['userid']>0){?><a href="<?php echo $data['Members']?>/member-Edinars/<?php echo $value['userid']?>/view"><?php echo prntext($value['username'])?></a><?}else{?><?php echo prntext($value['username'])?><?}?></td>
                                        ...
                                        <td nowrap>
                                        <?if($value['canview']){?>
                                            <a href="<?php echo $data['Members']?>/mon-historique-Edinars-transaction/<?php echo $value['id']?>/details">D&Egrave;TAIL</a><?}?>
                                                <?if($value['canrefund']){?>
                                                    <br><a href="<?php echo $data['Members']?>/mon-historique-Edinars-transaction/<?php echo $value['id']?>/refund" onclick="return cfmform()">REMBOURSE</a>
                                                <?}?>
                                        </td>
                                    </tr>
                                        <?php 
                    ...
                    <div class="page_area">
                        <?$count=count($data['Pages']);
                             for($i=0; $i<$count; $i++){?>
                                ...
                                <?}else{?>
                                    <a href="<?php echo $data['Members']?>/mon-historique-Edinars/<?php echo $data['Pages'][$i]?>/<?php echo $post['type']?>/<?php echo $post['status']?>/transaction" title="Page <?php echo $i+1?>"><?php echo $i+1?></a>
                                <?}?>
                            <?}?>
                    </div>            
                <?}?>
                                    ...
                                    <tr>    
                                        ...
                                        <td>    
                                            <?if($post['Transaction']['userid']>0){?>
                                                <a href="userinfo.htm?id=<?php echo $post['Transaction']['userid']?><?if(isset($post['StartPage'])){?>&page=<?php echo $post['StartPage']?><?}?>&action=view"><?php echo $post['Transaction']['username']?></a>
                                            <?}else{?>
