    ...
    <form method=post>
      <table class=frame width=500 border=0 cellspacing=1 cellpadding=4>
        ...
        <tr><td class=input><select name=template style="width:50%">
          <?php echo showselect($post['Templates'], $post['template'])?></select>
        </td>
        <td class=input>
      ...
      <table class=frame width=100% border=0 cellspacing=1 cellpadding=4>
        <tr><td class=capl colspan=2>CURRENT TEMPLATE IS: <?php echo $post['template']?></td></tr>
        <tr>
          ...
          <td class=capc width=1%>
            <input type=button class=submit 
                   onclick="window.open('<?php echo $data['Admins']?>/htmleditor/editor.htm',
                                        'Editor',
